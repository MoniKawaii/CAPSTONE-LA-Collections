"""
Harmonize Fact Orders from Multi-Platform API Response to Unified Schema
=========================================================================

This module processes order line items from both Lazada and Shopee and harmonizes 
them into the Fact_Orders table. The fact table contains one record per order line 
item (product within an order).

Platforms Supported:
- Lazada: lazada_orders_raw.json, lazada_multiple_order_items_raw.json
- Shopee: shopee_orders_raw.json, shopee_multiple_order_items_raw.json
...
- paid_price: Revenue for this individual line item
- original_unit_price: Non-discounted price per unit
- voucher_platform_amount: Platform voucher amount
- voucher_seller_amount: Seller voucher amount  
- shipping_fee_paid_by_buyer: Shipping fee for this line item
"""
...
def load_order_items_raw(platform='lazada'):
    """
    Load raw order items from JSON file for specified platform
    
    Args:
        platform (str): 'lazada' or 'shopee'
        
    Returns:
        list: Order items data
    """
    filename = f'{platform}_multiple_order_items_raw.json'
    json_path = os.path.join(os.path.dirname(__file__), '..', 'Staging', filename)
    
    try:
        with open(json_path, 'r', encoding='utf-8') as f:
            order_items_data = json.load(f)
        print(f"✓ Loaded {len(order_items_data)} raw {platform.capitalize()} order item records")
...
def load_lazada_order_items_raw():
    """Load raw Lazada order items from JSON file (legacy function)"""
    return load_order_items_raw('lazada')
...
def generate_time_key(date_string):
    ...
    """
def parse_order_date_to_time_key(date_value, platform='lazada'):
    """
    Convert order date to time_key (YYYYMMDD format)
    ...
    Returns:
        int: Time key in YYYYMMDD format
    """
    if not date_value:
    ...
    try:
        if platform == 'lazada':
            ...
            time_key = int(dt.strftime('%Y%m%d'))
            return time_key
    except (ValueError, AttributeError, IndexError, TypeError, OSError):
        print(f"⚠️ Warning: Could not parse date '{date_value}', using default")
        return 20220101  # Default fallback


def generate_platform_customer_id_lazada(first_name, phone):
    """
    Generate Lazada platform_customer_id using same logic as dim_customer
...
def generate_platform_customer_id_shopee(order_sn):
    """
    Generate Shopee platform_customer_id using order_sn
    Format: SP{OrderSN}
    """
...
def generate_platform_customer_id(first_name, phone):
    """Legacy function for Lazada"""
    return generate_platform_customer_id_lazada(first_name, phone)
...
def extract_order_items_from_shopee(order_items_data, orders_data, dim_order_df, dim_customer_df, dim_product_df, dim_variant_df):
    """
    Extract and harmonize fact order records from Shopee order items
    Creates one record per individual order item (no aggregation)
    
    Args:
        order_items_data: Raw Shopee order items data
        orders_data: Raw Shopee orders data (for customer info and dates)
        dim_order_df: Dimension table for order key lookup
        dim_customer_df: Dimension table for customer key lookup
        dim_product_df: Dimension table for product key lookup
        dim_variant_df: Dimension table for variant key lookup
        
    Returns:
        pd.DataFrame: Fact orders table with individual line items
    """
    fact_orders_records = []
    
    # Create lookup dictionaries for foreign keys
    order_key_lookup = dict(zip(dim_order_df['platform_order_id'].astype(str), dim_order_df['orders_key']))
    customer_key_lookup = dict(zip(dim_customer_df['platform_customer_id'].astype(str), dim_customer_df['customer_key']))
    product_key_lookup = dict(zip(dim_product_df['product_item_id'].astype(str), dim_product_df['product_key']))
    variant_key_lookup = dict(zip(dim_variant_df['platform_sku_id'].astype(str), dim_variant_df['variant_key']))
    
    print(f"📊 Lookup tables created:")
    print(f"   - Orders: {len(order_key_lookup)} mappings")
    print(f"   - Customers: {len(customer_key_lookup)} mappings")
    print(f"   - Products: {len(product_key_lookup)} mappings")
    print(f"   - Variants: {len(variant_key_lookup)} mappings")
    
    # Create lookup for order dates and customer info
    orders_lookup = {}
    for order in orders_data:
        order_sn = str(order.get('order_sn', ''))
        orders_lookup[order_sn] = {
            'create_time': order.get('create_time'),
            'buyer_username': order.get('buyer_username', '')
        }
    
    print(f"📊 Created order info lookup for {len(orders_lookup)} Shopee orders")
    
    order_item_key_counter = 1
    
    for order_record in order_items_data:
        try:
            platform_order_id = str(order_record.get('order_sn', ''))
            ...
            # Get customer key - Shopee uses order_sn for customer ID
            platform_customer_id = generate_platform_customer_id_shopee(platform_order_id)
            customer_key = customer_key_lookup.get(platform_customer_id)
            
            if customer_key is None:
                if order_item_key_counter <= 3:
                    print(f"⚠️ Warning: Could not find customer_key for {platform_customer_id}")
                continue
            
            # Get order info
            order_info = orders_lookup.get(platform_order_id, {})
            create_time = order_info.get('create_time')
            time_key = parse_order_date_to_time_key(create_time, platform='shopee') if create_time else 20220101
            
            # Process item_list
            item_list = order_record.get('item_list', [])
            ...
            for item in item_list:
                try:
                    ...
                    # Extract pricing - Shopee uses cents, convert to dollars
                    model_original_price = float(item.get('model_original_price', 0)) / 100
                    model_discounted_price = float(item.get('model_discounted_price', 0)) / 100
                    
                    # Get quantity
                    model_quantity_purchased = int(item.get('model_quantity_purchased', 1))
                    
                    # Calculate per-unit price
                    paid_price_per_unit = model_discounted_price if model_discounted_price > 0 else model_original_price
                    
                    # Get voucher amounts (if available, convert from cents)
                    voucher_platform = 0.0
                    voucher_seller = 0.0
                    
                    # Shopee doesn't expose item-level shipping in item_list
                    shipping_fee = 0.0
                    ...
                    fact_record = {
                        ...
                        'paid_price': paid_price_per_unit * model_quantity_purchased,
                        'original_unit_price': model_original_price,
                        'voucher_platform_amount': voucher_platform,
                        'voucher_seller_amount': voucher_seller,
                        'shipping_fee_paid_by_buyer': shipping_fee
                    }
            ...
            print(f"⚠️ Error processing order {order_record.get('order_sn', 'unknown')}: {e}")
            continue
...
def harmonize_fact_orders():
    """
    Main function to harmonize Fact Orders from both Lazada and Shopee API responses
    
    Returns:
        DataFrame: Harmonized fact orders table with data from both platforms
    """
    print("🚀 Starting Fact Orders harmonization (Multi-Platform)...")
    ...
    if dim_order_df.empty:
        ...
        print("   Please run harmonize_dim_order.py first")
    ...
    # Process Lazada order items
    print("\n📦 Processing Lazada order items...")
    lazada_order_items_data = load_order_items_raw('lazada')
    lazada_orders_data = load_orders_raw('lazada')
    ...
    if lazada_order_items_data and lazada_orders_data:
        ...
        if not lazada_fact_df.empty:
            all_fact_orders.append(lazada_fact_df)
            print(f"   ✓ Extracted {len(lazada_fact_df)} Lazada fact records")
    else:
        print("   ⚠️ No Lazada order items or orders data available")
    
    # Process Shopee order items
    print("\n🛍️ Processing Shopee order items...")
    shopee_order_items_data = load_order_items_raw('shopee')
    shopee_orders_data = load_orders_raw('shopee')
    
    if shopee_order_items_data and shopee_orders_data:
        shopee_fact_df = extract_order_items_from_shopee(
            shopee_order_items_data, shopee_orders_data, dim_order_df, dim_customer_df, dim_product_df, dim_variant_df
        )
        if not shopee_fact_df.empty:
            all_fact_orders.append(shopee_fact_df)
            print(f"   ✓ Extracted {len(shopee_fact_df)} Shopee fact records")
    else:
        print("   ⚠️ No Shopee order items or orders data available")
    
    # Combine all fact orders
    if not all_fact_orders:
        print("❌ No fact order data available from any platform")
        return get_empty_dataframe('fact_orders')
    
    fact_orders_df = pd.concat(all_fact_orders, ignore_index=True)
    
    print(f"\n✅ Successfully harmonized {len(fact_orders_df)} fact order records from both platforms")
    print(f"📊 Data shape: {fact_orders_df.shape}")
    
    if len(fact_orders_df) > 0:
        print("\n📊 Platform Breakdown:")
        lazada_count = len(fact_orders_df[fact_orders_df['platform_key'] == 1])
        shopee_count = len(fact_orders_df[fact_orders_df['platform_key'] == 2])
        print(f"   • Lazada fact records: {lazada_count}")
        print(f"   • Shopee fact records: {shopee_count}")
